...
public class TestPatientDataServiceRESTArq {
	private static Logger logger = Logger.getLogger(TestPatientDataServiceRESTArq.class.getName());

//	 server_wildfly_web_url/deployment_archive_name/ApplicationConfig_@ApplicationPath/EJB_@Path
	private static String serviceURL = "http://localhost:8080/msd-s5-test/data/patients";	
//	private static String serviceURL = "http://localhost:8080/MSD-S4/data/projects";	
	
	@Deployment // Arquilian infrastructure
	public static Archive<?> createDeployment() {
	        return ShrinkWrap
	                .create(WebArchive.class, "msd-s5-test.war")
	                .addPackage(Patient.class.getPackage())
	                .addPackage(Patient1DataService.class.getPackage())
	...
	public void test3_AddPatient() {
		...
		for (int i=1; i <= patientsToAdd; i++){
			patient = new Patient(i, "Patientt_" + (100 + i));
	...
	public void test3_CreatePatientt() {
		...
		Collection<Patient> projects = client.target(serviceURL)
				.request().get()
				.readEntity(new GenericType<Collection<Patient>>(){});
		
		assertTrue("Fail to add Patientts!", projects.size() >= patientssToAdd);
	}	
	@Test
	public void test2_DeletePatientt() {
		String resourceURL = serviceURL + "/";
		logger.info("DEBUG: Junit TESTING: test2_DeletePatients ...");
		Client client = ClientBuilder.newClient();
...
//	@Test
	...
	@Test
	public void test5_UpdatePatient() {
		String resourceURL = serviceURL + "/1"; //createNewProject
		logger.info("************* DEBUG: Junit TESTING: test5_UpdatePatient ... :" + resourceURL);
		Client client = ClientBuilder.newClient();
		// Get project
		...
		patient.setName(patient.getName() + "_UPD_JSON");
